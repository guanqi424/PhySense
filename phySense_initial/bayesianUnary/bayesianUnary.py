...
class BayesianUnary:
    ...
    def inference(self,
        ...
        if runtime_breakdown:
            ...
            return [predicted_size_x, predicted_size_y, predicted_size_z, predicted_probs_texture], size_time_end - size_time_start, lbp_time_end-lbp_time_start
        else:
            size_x_pd = pd.Series(size_x.cpu().numpy())
            size_x_pd = size_x_pd.clip(lower=self.size_x_range[0] + 1e-4, upper=self.size_x_range[1] - 1e-4)
            size_x_bin_x = pd.cut(size_x_pd, bins=self.linespace_x_bins, include_lowest=True)
