...
class phySense(torch.nn.Module):
    def __init__(self, *,
                 ...
                 small_labelspace=False, trust_frame_num=3,
                 workers_fraction=1., hashstr=None) -> None:
        super().__init__()
        ...
        self.bayesianUnary = BayesianUnary(file_path, num_size_x_bin, num_size_y_bin, num_size_z_bin,
                                           ...
                                           edge_buffer_ratio, num_regions, region_size_ratio, workers_fraction)
    ...
    def forward(self, filename_list, size_x, size_y, size_z,  # [batch_size, num_nodes_max]
        ...
        if self.use_precompute:
            ...
            if self.hashstr is not None:
                targetstr = str(filename_list) + self.hashstr
            else:
                targetstr = str(filename_list)
            ...
            if os.path.isfile('./lbp_precompute/' + unaries_hash + '.pkl'):
                with open('./lbp_precompute/' + unaries_hash + '.pkl', 'rb') as file:
                    try:
                        ...
                        print('./lbp_precompute/' + unaries_hash + '.pkl')
            else:
...
class GraphDataset_addgt(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(gt_labels)):
            if gt_labels[i] not in self.label_encoder.classes_:
                # print(gt_labels[i])
                gt_labels[i] = None
        
        encoded_gtlabels = torch.tensor(self.label_encoder.transform(gt_labels), dtype=torch.long)

        return beh_seq, edges, size_x, size_y, size_z, interactions, graph_nums, encoded_labels, filenames, encoded_gtlabels
