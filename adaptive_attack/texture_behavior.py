    ...
    def inference(self, indices, filenames, size_xs, size_ys, size_zs, padded_beh_seqs, lengths, batch_masks,
        ...
        with torch.no_grad():
            ...
            outputGuard, times, texture_prob, behavior_prob = self.phySense.decode(filenames, size_xs, size_ys, size_zs, padded_beh_seqs, lengths, batch_masks,
                                                           batch_edges, interactions, graph_nums, batch_edge_masks, self.device, None)
            output_guard = []
            texture_prob_res = []
            behavior_prob_res = []
...
def obj_func(solution_tensor, part_start, part_end):
    ...
    for scene_num, frames_len in tqdm(enumerate(part_lengths[part_start:part_end]), total=part_end-part_start):
        for cur_frame_num in range(frames_len):
            ...
            for i in range(len(outputYOLO)):
                if outputYOLO[i] not in coco_to_labels:
                    ...
                    missed += torch.min(texture_prob[i]).item()
                    missed += weight_beh1 * torch.min(behavior_prob[i].sum(axis=1)).item()
                else:
                    ...
                    if perception_result != 2:
                        ...
                        # if not bypassed
                            ...
                            missed += texture_prob[i][perception_result].item()
                            missed += weight_beh2 * behavior_prob[i][perception_result].sum().item()
                    # if attack fail, add punish
                    else:
                        missed -= texture_prob[i][2].item()
                        missed -= weight_beh3 * behavior_prob[i][2].sum().item()
    return missed / total_pert

def fitness_func(ga_instance, solution, solution_idx):
    solution_tensor = torch.tensor(solution.reshape((3, 600, 600)), dtype=torch.float32, device=device)
    
    return obj_func(solution_tensor, 0, 5)
...
def on_gen(ga_instance):
    ...
    showtensor(solution_tensor, save_dir=f'./adaptive_attack/pygad_sol_texture_behavior/{ga_instance.generations_completed}.png')

os.makedirs('./adaptive_attack/gad_pert/', exist_ok=True)
os.makedirs('./adaptive_attack/pygad_sol_texture_behavior/', exist_ok=True)
...
np.save('./adaptive_attack/fitness_values_texture_behavior.npy', ga_instance.best_solutions_fitness)

ga_instance.plot_fitness(save_dir='./adaptive_attack/pygad_fitness_texture_behavior.png')

solution, solution_fitness, solution_idx = ga_instance.best_solution()
np.save('./adaptive_attack/best_solution_texture_behavior.npy', solution)
print(f"Parameters of the best solution : {solution}")
print(f"Fitness value of the best solution = {solution_fitness}")
print(f"Index of the best solution : {solution_idx}")

solution_tensor = torch.tensor(solution.reshape((3, 600, 600)), dtype=torch.float32)
showtensor(solution_tensor, save_dir='./adaptive_attack/pygad_sol_texture_behavior.png')
ga_instance.save(filename='./adaptive_attack/pygad_instance_texture_behavior')
