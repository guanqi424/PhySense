...
def test_model(model, test_loader, label_dict, device=torch.device("cpu"), small_labelspace = False):
    ...
    print(classification_report(new_labels, new_preds, labels=range(10), target_names=target_names_label, zero_division=0))

    plt.figure(figsize=(12, 12))
    cm = confusion_matrix(new_labels, new_preds)
    sns.heatmap(cm, annot=True, fmt="d", linewidths=.5, cmap="YlGnBu",
                xticklabels=target_names_label, yticklabels=target_names_label)
    plt.xlabel('Predicted labels')
    plt.ylabel('True labels')
    plt.title('Confusion Matrix (per label)')
    plt.xticks(rotation=45, ha="right", fontsize=10)
    plt.yticks(fontsize=10)
    plt.tight_layout()
    plt.show()
