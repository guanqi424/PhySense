...
def main(runtime_test=False, save_interval=1, save_dir='./data/carla/crf_dataset_train/', save_parts=1):
    ...
    carla_space = ['bicycle', 'car', 'pedestrian']
    ...
    num_scenes = len(scene_name_dict.items())
    dataset_to_show = None
    crf_idx_to_scene = {}
    for idx, (scene_name, scene_id) in tqdm(enumerate(scene_name_dict.items()), total=len(scene_name_dict.items())):
        crf_idx_to_scene[idx] = (scene_name, scene_id)
        start_time = time.time()
        full_instances = {key: value for key, value in final_dataset_gt.items() if key.startswith(scene_name) and any(sub in key for sub in carla_space)}
        timestamp_list = []
        ...
        for framenum in timestamp_list:
            ...
            if len(my_graph) > 0:
                ...
                for node in my_graph:
                    exist_label_set.add(node[1]['name'])
                    my_graph_refined[1].append(node[1]['name'])
                    if 'gt_aligned' in node[1]:
                        assert node[1]['gt_aligned']
                    my_graph_refined[2].append(True if node[1]['name'] in node[0] else False)
                ...
                for i in range(len(my_graph_refined[1])):
                    for j in range(len(my_graph_refined[1])):
                        ...
                        isConnected, possibleInteractions, inference_time = inference_two_nodes(node1, node2, threshold_deltaTTC=1,
                                                                                                threshold_maxTTC=2, radius=2,  # radius removed
                                                                                                threshold_diffVp=1,
    ...
    if not runtime_test:
        ...
        for graph in dataset_to_show[44:]:
            if len(graph[0]['edges']) != 0:
                print('Node', graph[0]['edges'][1][0], 'to', 'Node', graph[0]['edges'][1][1])
                print("Groundtruth Labels:", graph[1][graph[0]['edges'][1][0]], graph[1][graph[0]['edges'][1][1]])
                print('Potential object label pair inferred from interactions:')
                for interaction in graph[0]['possibleInt'][1]:
...
if __name__ == '__main__':
    os.makedirs('./data/carla/crf_dataset_train/', exist_ok=True)
    main()
